...
- 位运算：& | ^ ~ << >>
  ...
  // m = 6 (110), n = 3 (011)
  print(m & n); // 2 (010)  按位与：110 & 011 = 010
  print(m | n); // 7 (111)  按位或：110 | 011 = 111
  print(m ^ n); // 5 (101)  按位异或：110 ^ 011 = 101
  print(~m);    // -7      按位取反：~110 = ...1111111111111001（二进制补码，结果为-7）
  print(m << 1); // 12   二进制位向左移动，低位补0，相当于乘以2的n次方：110 << 1 = 1100（二进制），即 6*2=12
  print(m >> 1); // 3    二进制位向右移动，高位补符号位，相当于除以2的n次方：110 >> 1 = 011（二进制），即 6/2=3
  ```
- 类型判断：is、as
  ```dart
  var s = 'hello';
...
- 赋值：=、??=、+=、-= 等
  ```dart
  int v;         // 声明一个 int 类型变量 v
  v = 5;         // 赋值，v 现在是 5
  v += 2; // 7   // 相当于 v = v + 2，结果为 7
  v -= 3; // 4   // 相当于 v = v - 3，结果为 4
  int? w;        // 声明一个可空整型变量 w，初始为 null
  w ??= 10;      // 如果 w 为 null，则赋值为 10；如果 w 已有值则不变
  ```
- 条件：?:
  ```dart
  int age = 20;
